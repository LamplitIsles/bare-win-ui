...
module.exports = exports = class WinUIWebView extends WinUIElement {
  ...
  navigate(url) {
    return this._enqueue(() => binding.webViewNavigate(this._handle, url))
  }

  navigateToString(html) {
    return this._enqueue(() => binding.webViewNavigateToString(this._handle, html))
  }

  postMessage(message) {
    return this._enqueue(() => binding.webViewPostMessage(this._handle, message))
  }

  openDevToolsWindow() {
    return this._enqueue(() => binding.webViewOpenDevToolsWindow(this._handle))
  }
  ...
  _enqueue(operation) {
    const run = this._queue
      .then(() => this._ready.promise)
      .then(() => {
        if (!this._destroyed) operation()
        return this
      })
